...
- 每 Batch 必须通过质量契约检查（具体检查项见 `docs/03-quality-contract.md`）
- Batch 验收前跑质量检查命令必须全绿

### 3.4 未知信息纪律

遇到不确定的事实/数据/API，**先查证**；不要靠记忆编造。

### 3.5 决策纪律

- 小决策（工具选型、文件组织）：自主决定 → 追加写入 `DECISIONS.md`（带日期 + 理由）
- 大决策（方向变更、不可逆选型）：找用户确认
...
- 大批量独立子任务用 `Agent` 并行
- 串行瓶颈任务不要并行
- 卡住 15 分钟未解 → spawn rescue agent 做独立诊断
...
│   ├── 02-architecture.md      （Phase 1 项目架构）
│   ├── 03-quality-contract.md  （Phase 1 质量契约）
│   └── execution/
│       ├── README.md
│       ├── PROGRESS.md         （进度看板）
│       ├── DECISIONS.md        （决策日志）
...
- **如果架构已定稿但 PROGRESS.md 还没建**：进入 Phase 2 拆 Batch
